...
import { getTasks, deleteTask, updateTaskPosition, createStatus } from '@/lib/api';
...
export default function Board() {
  ...
  const getColumnTasks = useCallback(
    (statusId) => state.tasks.filter((t) => t.status_id === statusId).sort((a, b) => a.position - b.position),
    [state.tasks]
  );
  ...
  const handleDragEnd = useCallback(async (result) => {
    ...
    const destTasks = state.tasks.filter((t) => t.status_id === newStatusId && t.id !== draggableId).sort((a, b) => a.position - b.position);
    ...
    dispatch({ type: 'SET_TASKS', payload: [...state.tasks.filter((t) => t.id !== draggableId), { ...task, status_id: newStatusId, position: optimisticPos }] });
    mutationInFlightRef.current = true;
    try {
      await updateTaskPosition(activeWorkspace.id, draggableId, newStatusId, prevPos, nextPos);
    } catch (err) {
      dispatch({ type: 'SET_TASKS', payload: prev });
    ...
    return (
      <div className="flex gap-5 max-w-6xl mx-auto px-6 pb-8">
        {(statuses.length > 0 ? statuses : [1, 2, 3]).map((s, idx) => (
          <div key={s.id || idx} className="flex flex-col w-[300px] min-w-[300px] rounded-xl bg-[#f4f4f2] border border-[#e4e4e0] p-4 h-60 animate-pulse" />
        ))}
      </div>
    );
  }
        ...
        <div className="flex gap-5 max-w-6xl mx-auto px-6 pb-8 overflow-x-auto items-start">
          ...
          <AddColumnButton 
            onAdd={async (name) => {
              if (!activeWorkspace?.id || !activeProject?.id) return;
              try {
                await createStatus(activeWorkspace.id, activeProject.id, { name, color: '#9ca3af', position: statuses.length * 65536 });
                refreshStatuses();
              } catch (err) {
  ...
  return (
    <div className="w-[300px] min-w-[300px] bg-white border border-[#e4e4e0] rounded-xl p-4 shadow-sm animate-in fade-in slide-in-from-top-2 duration-200">
      <Input
        ...
        onKeyDown={(e) => {
          if (e.key === 'Enter' && name.trim()) { onAdd(name.trim()); setName(''); setIsEditing(false); }
      ...
      <div className="flex items-center gap-2">
        <Button 
          size="sm" 
          onClick={() => { if (name.trim()) { onAdd(name.trim()); setName(''); setIsEditing(false); } }}
