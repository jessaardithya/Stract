...
// ─── Prop Item ────────────────────────────────────────────────────────────────
function PropItem({ label, children }) {
  return (
...
export default function TaskDetailModal() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (!members.length)
          setMembers((await getMembers(activeWorkspace.id)) || []);
        if (!labels.length)
          setLabels((await getLabels(activeWorkspace.id)) || []);
      } catch (err) {
  ...
  const handleAddSubtask = async () => {
    if (!newSubtask.trim() || !activeWorkspace) return;
    try {
        ...
        {loading ? (
          ...
          >
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-2 truncate">
                      <span
                        className={`w-1.5 h-1.5 rounded-full shrink-0
                                        ${PRIORITY_CFG[task?.priority]?.dot || "bg-zinc-300"}`}
                      />
                      ...
                      <CommandList>
                        ...
                        <CommandGroup>
                          ...
                          {members.map((m) => (
                            <CommandItem
                              key={m.id}
                              onSelect={() => patchTask({ assignee_id: m.id })}
            ...
            <div className="mt-8 pt-6 border-t border-black/[0.06] space-y-2.5">
              ...
              <MetaRow
                label="Updated"
                value={task?.updated_at ? formatRelative(task.updated_at) : "—"}
              />
